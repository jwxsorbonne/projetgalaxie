...
class GalaxyNumba:
    ...
    def step(self, dt):
        ...
        if self.use_parallel:
            a = compute_forces_parallel(self.positions, self.masses)
        else:
            a = compute_forces_numba(self.positions, self.masses)
            
        self.positions += self.velocities * dt + 0.5 * a * (dt**2)
        
        if self.use_parallel:
            a_new = compute_forces_parallel(self.positions, self.masses)
        else:
            a_new = compute_forces_numba(self.positions, self.masses)
